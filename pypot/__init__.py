...
class HoneyPot(object):
    ...
    def handle_connection(self, client_socket, port, ip, remote_port):
        self.logger.info("Connection received: %s: %s:%d" % (port, ip,int(remote_port)))

        client_socket.settimeout(5)
        try:
            data = client_socket.recv(64)
            self.logger.info("Data received: %s: %s:%d: %s" % (port, ip, int(remote_port), data))
    ...
    def start_listening(self):
        for port in self.ports:
            self.listeners_threads[port] = threading.Thread(target=self.start_new_listener_thread, args=(port,))
            self.listeners_threads[port].start()
